...
- [x] **T2 — `pi` tab in the UI.** Route: delegated writer. New `PiView.tsx` modelled on `ClaudeView.tsx`, a `pi` entry in `V3Tab` + `TOPBAR_NAV`, wiring in `AppV3.tsx`, and en/es i18n keys. Shows each row with an explicit ok / missing / unknown state, and names the fix in the copy when the binary is missing.

- [x] **T3 — "Open in Pi" launcher.** Route: delegated writer. Added at the user's request after T1 and T2 landed: opening Pi in a project from CSK is what they actually wanted first, so it takes priority over the remediation slice. A `open_in_pi` Rust command opens a terminal in the selected project directory running `pi`, modelled on `open_in_tui` (validate the path, resolve the executable, prefer Windows Terminal with the repo's existing fallback), plus a button in the Projects row next to "Open in TUI" with its i18n keys.
...
- **Not yet verified:** the command has never been invoked at runtime, because that needs `pnpm tauri dev`.
- T1 shipped as `af9f396`. Native review: assessed `high` (`process_boundary`, `signal: shell_process` on `src-tauri/src/lib.rs`), consent relayed, and the user **declined this candidate**. The decline is candidate-scoped, nothing was persisted, and reviews stay enabled.
- **T2 done.** `src/views/v3/PiView.tsx` (new, 294 lines) plus `v3types.ts`, `v3Nav.ts`, `AppV3.tsx` and `i18n.ts` (+51/-1). The tab reuses ClaudeView's fetch-on-mount pattern and pill vocabulary, adds no CSS, and surfaces the payload's `errors` as a visible list so a backend failure cannot read as an empty state. 20 `pi.*` keys plus `nav.pi`, in `en` and `es`. Check of record: `pnpm typecheck` → clean, exit 0 (re-run as a parent spot check). There is no JS test runner in this repo, so T2 carries no unit tests.
- T2 shipped as `99a64e2`. Assessed against `af9f396` as the base: `medium` (`executable_change` on `src/constants/v3Nav.ts`), `review_due: false`, reason `under_budget` (355 authored lines).
- **That assessment used the wrong base, and the record above is corrected here.** A declined candidate does not advance the reviewed boundary, so the boundary stayed at the branch point and the real candidate was the whole accumulated branch: 7 paths, 922 lines, `high` on the same `process_boundary` evidence. The stop hook caught it. The selectorless STATUS preflight was run, its exact START executed, the consent envelope relayed, and the user **declined that candidate too** (`declined_this_candidate`, target `sha256:aa7bb6a0…`). Nothing was persisted and reviews stay enabled. Rule for the next slice: assess with `--base-ref` at the last *acknowledged* boundary — here still the branch point `d356808` — never at the last commit.
- **Still open, and it needs a human:** neither the command nor the panel has been exercised at runtime. Run `./dev.ps1` and open the Pi tab to close the remaining acceptance criteria — the live payload, the missing-binary row, and the malformed-settings path.
- **T3 done.** `src-tauri/src/lib.rs` (+151), `src/views/v3/ProjectsView.tsx` (+14), `src/lib/i18n.ts` (+1). Observed RED first (`cannot find function \`build_open_in_pi_launch\` in this scope` at three call sites), then GREEN. `cargo test --locked` → 93 passed, `cargo clippy -D warnings` → clean, `pnpm typecheck` → exit 0, all re-run as parent spot checks. The pure, tested piece is `build_open_in_pi_launch`, which builds `wt new-tab --title Pi --startingDirectory <dir> cmd /k <pi>` when Windows Terminal resolves and plain `cmd /k <pi>` otherwise. Both branches go through `cmd /k` so cmd.exe resolves the `.cmd` wrapper and the window survives an error. `open_in_pi` deliberately uses raw `std::process::Command` instead of `silent_command()`, because `CREATE_NO_WINDOW` is exactly what would stop the console from appearing. Process spawning and Windows Terminal resolution are not unit tested; faking that would prove nothing.
- **Still unexercised at runtime:** the panel, the button, the Windows Terminal branch and the "pi not found" error path. None of it has been clicked.
- Next step: the remediation actions slice (rebuild the binary, flag conflicting packages).
