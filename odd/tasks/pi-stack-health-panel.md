...
- T2 shipped as `99a64e2`. Native review: assessed `medium` (`executable_change` on `src/constants/v3Nav.ts`), `review_due: false`, reason `under_budget` (355 authored lines against the ~400 budget), so it stays pending in this slice until a later commit reaches it. The base for this assessment is `af9f396` rather than the branch point, because the range up to `af9f396` was already offered for review and declined; re-offering a superset of a declined candidate would re-open a settled decision.
...
- Next step: runtime check, then the remediation actions slice (rebuild the binary, flag conflicting packages).
